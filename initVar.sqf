...
if (!hasIFA) then
	...
	//NON-IFA Templates for DEFENDER
		if (!activeUSAF) then
			...
			call compile preProcessFileLineNumbers "Templates\Occupantscup.sqf";
			}
			else
			{
				if (has3CB) then
		...
		if (!activeAFRF) then
			...
			call compile preProcessFileLineNumbers "Templates\Invaderscup.sqf";
			}
			else
			{
				if (has3CB) then
		...
		if (!activeGREF) then
			...
			call compile preProcessFileLineNumbers "Templates\teamplayercup.sqf";
			}
			else
			{
				if (has3CB) then
